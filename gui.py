...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.file_textbox = customtkinter.CTkEntry(self.ofd_frame,width=400, height =35, placeholder_text="Selectionner Un Fichier...", corner_radius=5, border_color="#EEE", fg_color="transparent", text_color="#0774be")
        ...
        ## 2 - Process Frame And Elements ##
        self.process_frame = customtkinter.CTkFrame(self, corner_radius=8, border_width = 1, border_color="#CCC", fg_color="transparent")
        self.process_frame.grid(row = 3, column = 0, columnspan = 2, sticky="ew", padx=20, pady = 0)
        self.process_frame.grid_columnconfigure(0, weight=1)  # Label column - fixed width
        self.process_frame.grid_columnconfigure(1, weight=1)  # Label column - fixed width


        self.print_radio = customtkinter.CTkRadioButton(self.process_frame,text ="Impression des Factures",border_width_unchecked = 2, border_width_checked = 6,
            border_color = "#0774be",
            variable = self.radio_option,
            value ="print",
            hover = True)

        self.pns_radio = customtkinter.CTkRadioButton(self.process_frame,text ="Extraction des PNS", border_width_unchecked = 2, border_width_checked = 6,
            border_color="#0774be",
            variable = self.radio_option,
            value="pns",
            hover = True)

        self.print_radio.grid(row=0, column=0, padx=20, pady=(10, 0), sticky = "w")
        self.pns_radio.grid(row=0, column= 1, padx=20, pady= (10,0), sticky = "e")

        self.advanced_label = CTkLabel(self.process_frame, text = u'\u25B6 Options Avancées', text_color="#0774be", font=CTkFont(weight="bold", size=12), cursor="hand2")
        self.advanced_label.grid(row=1, column= 0, padx=20, pady= (0,8), sticky = "w")
        self.advanced_label.bind("<Button-1>", self.toggle_advanced_frame)

        self.advanced_frame = CTkFrame(self.process_frame, corner_radius=8, fg_color="transparent")
        self.advanced_frame.grid_columnconfigure(0,weight=1)

        self.num_client = customtkinter.CTkEntry(self.advanced_frame, height =30, width=200, placeholder_text="N° Clent", corner_radius=5, border_color="#EEE", fg_color="transparent", text_color="#0774be")
        self.logo_check = customtkinter.CTkCheckBox(self.advanced_frame, text="Imprimer avec Logo", border_color="#0774be",border_width=2,variable=self.check_value,hover=True)
        self.date_debut = customtkinter.CTkEntry(self.advanced_frame, height=30, width=200, placeholder_text="MM/AAAA", corner_radius=5,border_color="#EEE", fg_color="transparent", text_color="#0774be")
        self.date_fin = customtkinter.CTkEntry(self.advanced_frame, height=30, width=200, placeholder_text="MM/AAAA", corner_radius=5,border_color="#EEE", fg_color="transparent", text_color="#0774be")

        self.num_client.grid(row=0, column= 0, padx=8, pady= 5, sticky = "w")
        self.logo_check.grid(row=0, column= 1, padx=8, pady= 5, sticky = "w")
        self.date_debut.grid(row=1, column= 0, padx=8, pady= 5, sticky = "w")
        self.date_fin.grid(row=1, column= 1, padx=8, pady= 5, sticky = "w")


        # 3 Actions Frame and Elements
        self.actions_frame = customtkinter.CTkFrame(self, corner_radius=10,fg_color="#F9F9F9")
        self.actions_frame.grid(row=4, column=0, columnspan=2, sticky="ew", padx=20, pady=10)

        self.actions_frame.grid_rowconfigure(1, weight=2)

        ## Distribute buttons
        for i in range (4): self.actions_frame.grid_columnconfigure(i, weight=1)

        #Calling a Handler for each Action
        ...
        self.state_frame = customtkinter.CTkFrame(self, corner_radius=5, fg_color="transparent")
        self.state_frame.grid(row=5, column=0, padx= 20, pady=(0,10), sticky = "ew", columnspan = 2)
        ...
        fu.initiate_wf(self)
        self.after(100, lambda: self.advanced_frame.configure(width=self.process_frame.winfo_width()))
...
# Toggle function
    def toggle_advanced_frame(self, event=None):
        if self.advanced_visible:
            self.advanced_frame.grid_remove()
            self.advanced_label.configure(text=u'\u25B6 Options Avancées')  # collapsed icon
        else:
            self.advanced_frame.grid(row = 2, column = 0, padx=20, pady = (0,8), sticky ="ew",  columnspan = 2)
            self.advanced_label.configure(text=u'\u25BC Options Avancées')  # expanded icon
